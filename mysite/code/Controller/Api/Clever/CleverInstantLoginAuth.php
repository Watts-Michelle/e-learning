...
class CleverInstantLoginAuth_Controller extends Base_Controller {
/**
 * This script demonstrates Clever Instant Login, using OAuth 2.0 for token acquisition & request authentication.
 */
    ...
    protected $redirect_uri = 'http://studytracks.dev/api/clever/auth/login';
    ...
    public function CleverLogin(SS_HTTPRequest $request)
    {
        if($code = $request->getVar('code')){
            $array = $this->process_incoming_requests($code, $this->set_options());
...
//        return (new JsonApi)->formatReturn([]);
    ...
    function set_options(array $override_options = NULL)
    {
        $options = array(
            ...
            'clever_redirect_base' => $this->redirect_uri,
            'clever_oauth_base' => 'https://clever.com/oauth',
            'clever_api_base' => 'https://api.clever.com/v1.2',
        );
        ...
        $options['clever_oauth_authorize_url'] = $options['clever_oauth_base'] . "/tokens";
...
//        $options['client_redirect_url'] = $options['clever_redirect_base'] . "/oauth";
        $options['client_redirect_url'] = $options['clever_redirect_base'];
     ...
     */
    function process_incoming_requests($code, array $options)
    {
//        if(preg_match('/auth/', $incoming_request_uri)) {
        if($code) {
            try {
                $me = $this->process_client_redirect($code, $options);
